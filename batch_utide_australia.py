...
NPZ_DIR = Path("/home/oliver/water_levels/Australia_BOM/npz")
TEMPLATE_PATH = Path("/home/oliver/harmonics_working/harmonics-dwf-20070318_no_us_no_dupes.txt")
OUTPUT_PATH = Path("/home/oliver/harmonics_working/harmonics_utide_australia.txt")
CHECKPOINT_DIR = Path("/home/oliver/harmonics_working/checkpoints_au")
...
def format_station_block(result):
    ...
    lines = []
    lines.append(f"# Harmonic constants derived from BOM ABSLMP hourly observation data")
    ...
    lines.append(f"# source: Derived from BOM ABSLMP observed data with UTide harmonic analysis")
    lines.append(f"# station_id_context: BOM-{station['station_id']}")
    lines.append(f"# date_imported: {datetime.now().strftime('%Y%m%d')}")
    lines.append(f"# datum: AHD")
...
def main():
    ...
    print("=" * 70)
    print("UTide Harmonic Analysis — Australia (BOM ABSLMP)")
    print(f"Stündliche Beobachtungen, AHD-Datum, letzte {USE_LAST_N_YEARS} Jahre")
    print("=" * 70)
