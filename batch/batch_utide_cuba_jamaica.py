#!/usr/bin/env python3
"""
UTide harmonic analysis for Caribbean tide stations (Cuba + Jamaica + Bahamas)
using UHSLC hourly data. Uses last 19 years (full nodal cycle) where available.
"""
import sys
...
CSV_DIR = Path("/home/oliver/water_levels/Caribbean_UHSLC_CubaJamaicaBahamas")
TEMPLATE_PATH = Path("/home/oliver/harmonics/classic/harmonics-dwf-20070318_mod.txt")
OUTPUT_PATH = Path("/home/oliver/harmonics/utide/harmonics_utide_caribbean_uhslc.txt")
CHECKPOINT_DIR = Path("/home/oliver/harmonics/utide/checkpoints_cuba_jamaica")

USE_LAST_N_YEARS = 19

STATIONS = {
    244:    {'name': 'Gibara',           'lat': 21.117, 'lon': -76.117, 'country': 'Cuba',      'tz': 'America/Havana',    'gloss_id': None, 'years_to_use': None,                                                'source': 'UHSLC'},
    # Guantanamo: dense 1937-1948, sparse 1995-1997. Combining all eras adds
    # ~5 cm RMS (datum shift / MSL rise) — restrict to 1937-1948 block.
    251:    {'name': 'Guantanamo Bay',   'lat': 19.907, 'lon': -75.147, 'country': 'Cuba',      'tz': 'America/Havana',    'gloss_id': None, 'years_to_use': None, 'date_range': ('1937-01-01', '1949-01-01'), 'source': 'UHSLC'},
    782:    {'name': 'Port Royal',       'lat': 17.933, 'lon': -76.850, 'country': 'Jamaica',   'tz': 'America/Jamaica',   'gloss_id': 210,  'years_to_use': None,                                                'source': 'UHSLC'},
    # Settlement Point: datum jump ~2m between 2001 and 2003 — restrict to
    # post-jump period 2003-2018 (16 years, close to nodal cycle).
    257:    {'name': 'Settlement Point', 'lat': 26.690, 'lon': -78.983, 'country': 'Bahamas',   'tz': 'America/Nassau',    'gloss_id': 211,  'years_to_use': None, 'date_range': ('2003-01-01', '2019-01-01'), 'source': 'UHSLC'},
    # Puerto Barrios: IOC SLM radar sensor, 2014-2018 only (post-2018 sensor
    # unstable). Data pre-processed with 30-day rolling-median datum correction
    # to remove recurring datum shifts within the stable window.
    'prba': {'name': 'Puerto Barrios',   'lat': 15.695, 'lon': -88.622, 'country': 'Guatemala', 'tz': 'America/Guatemala', 'gloss_id': None, 'years_to_use': None, 'source': 'IOC'},
}
...
def format_station_block(result):
    ...
    source = meta.get('source', 'UHSLC')
    if source == 'IOC':
        lines.append(f"# source: Derived from IOC SLM hourly radar data with UTide harmonic analysis")
        lines.append(f"# station_id_context: IOC-{result['uhslc_id']}")
    else:
...
def main():
    ...
    print("=" * 70)
    print("UTide Harmonic Analysis — UHSLC Hourly Data (Cuba + Jamaica)")
