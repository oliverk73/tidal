#!/usr/bin/env python3
"""
UTide harmonic analysis for New Brunswick tide stations.
Uses WLP (CHS predictions) data — reverse-engineering CHS harmonics.
Identical method to PEI (batch_utide_pei.py).
"""
import sys
...
NB_DIR = Path("/home/oliver/water_levels/Canada_IWLS_NB")
TEMPLATE_PATH = Path("/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt")
OUTPUT_PATH = Path("/home/oliver/harmonics/utide/harmonics_utide_nb.txt")
CHECKPOINT_DIR = Path("/home/oliver/harmonics/utide/checkpoints_nb")
...
def format_station_block(result):
    ...
    lines.append(f"# province: New Brunswick")
    ...
    lines.append(f"{name}, New Brunswick, Canada")
...
def main():
    ...
    with open(NB_DIR / '_nb_stations.json') as f:
        catalog = json.load(f)
    catalog_by_code = {s['code']: s for s in catalog}

    # Find all WLP CSV files
    wlp_files = sorted(glob.glob(str(NB_DIR / '*_wlp.csv')))

    print("=" * 70)
    print("UTide Harmonic Analysis — New Brunswick (CHS WLP)")
    print("=" * 70)
