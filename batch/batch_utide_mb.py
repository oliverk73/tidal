#!/usr/bin/env python3
"""UTide harmonic analysis for Manitoba coastal stations (Hudson Bay)."""
...
q.QC_DIR = Path("/home/oliver/water_levels/Canada_IWLS_MB")
q.TEMPLATE_PATH = Path("/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt")
q.OUTPUT_PATH = Path("/home/oliver/harmonics/utide/harmonics_utide_canada_mb.txt")
q.CHECKPOINT_DIR = Path("/home/oliver/harmonics/utide/checkpoints_mb")

ORIG = q.format_station_block
def fmt(r):
    return ORIG(r).replace('# province: Québec','# province: Manitoba') \
                  .replace(', Québec, Canada',', Manitoba, Canada')
q.format_station_block = fmt

def main():
    q.CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)
    with open(q.QC_DIR/'_mb_stations.json') as f: cat=json.load(f)
    by_code={s['code']:s for s in cat}
    files=sorted(glob.glob(str(q.QC_DIR/'*_wlp.csv')))
    print(f"UTide — Manitoba ({len(files)} stations)\n")
    import utide
    ...
    if results:
        ...
        for r in results:
            print(f"  {r['name']:<25s} {r['code']}  R²={r['r_squared']:.4f} M2={r['m2_amp']:.4f}")
