#!/usr/bin/env python3
"""UTide harmonic analysis for Belgian MVB tidal currents.

Mirrors batch_utide_nl_currents.py. Speed unit from MVB is m/s, converted to knots.
"""
...
CSV_DIR       = Path('/home/oliver/currents/Belgium')
META_PATH     = CSV_DIR / '_stations.json'
TEMPLATE_PATH = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt')
OUTPUT_PATH   = Path('/home/oliver/harmonics/utide/harmonics_utide_belgium_currents.txt')
CHECKPOINT    = Path('/home/oliver/harmonics/utide/checkpoints_be_currents')
...
def fmt_block(r):
    name = r['name']
    L = []
    L.append(f"# Harmonic constants derived from Meetnet Vlaamse Banken ADCP cell 3")
    ...
    L.append(f"#")
    L.append(f"# {name}, Belgium Current")
    L.append(f"# BEGIN HOT COMMENTS")
    L.append(f"# country: Belgium")
    L.append(f"# source: Meetnet Vlaamse Banken (MVB) V2 API, UTide analysis")
    L.append(f"# station_id_context: MVB-{r['code']}")
...
def main():
    ...
    print(f"MVB Currents — {len(csvs)} CSVs\n")
