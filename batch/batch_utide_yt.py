#!/usr/bin/env python3
"""UTide harmonic analysis for Yukon CHS stations."""
...
q.QC_DIR = Path("/home/oliver/water_levels/Canada_IWLS_YT")
q.TEMPLATE_PATH = Path("/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt")
q.OUTPUT_PATH = Path("/home/oliver/harmonics/utide/harmonics_utide_canada_yt.txt")
q.CHECKPOINT_DIR = Path("/home/oliver/harmonics/utide/checkpoints_yt")

ORIG = q.format_station_block
def fmt(r):
    return ORIG(r).replace('# province: Québec','# province: Yukon') \
                  .replace(', Québec, Canada',', Yukon, Canada')
q.format_station_block = fmt

def main():
    q.CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)
    with open(q.QC_DIR/'_yt_stations.json') as f: cat=json.load(f)
    by_code={s['code']:s for s in cat}
    files=sorted(glob.glob(str(q.QC_DIR/'*_wlp.csv')))
    print(f"UTide — Yukon ({len(files)} stations)\n")
    import utide
