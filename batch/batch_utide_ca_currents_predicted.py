#!/usr/bin/env python3
"""UTide harmonic analysis for CHS predicted currents (wcsp1 + wcdp1).

Produces UTide TC group entries (derived from official tide-current
predictions). CHS predictions are 1-min cadence; we subsample to 6-min
(stride 6) before analysis to keep UTide tractable while still resolving
shallow-water constituents up to M8.
"""
...
CSV_DIR       = Path('/home/oliver/currents/Canada_CHS_predicted')
META_PATH     = CSV_DIR / '_stations.json'
TEMPLATE_PATH = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt')
OUTPUT_PATH   = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_currents_predicted.txt')
CHECKPOINT    = Path('/home/oliver/harmonics/utide/checkpoints_ca_currents_predicted')
...
SUBSAMPLE_STRIDE = 6  # 1-min -> 6-min
...
def analyze(code, meta, csv_path):
    ...
    print(f"  {len(df)} Punkte (stride {SUBSAMPLE_STRIDE}), Hauptachse {axis:.1f}°, |v|max={np.abs(scal).max():.2f}kn")
...
def fmt_block(r):
    name = r['name']
    L = []
    L.append(f"# Harmonic constants derived from CHS IWLS predicted currents (wcsp1+wcdp1)")
    L.append(f"# (UTide v{utide.__version__}, principal-axis projection, 6-min subsample)")
    ...
    L.append(f"# source: Canadian Hydrographic Service (CHS) IWLS API predictions, UTide analysis")
...
def main():
    ...
    print(f"CHS predicted currents — {len(csvs)} CSVs\n")
