#!/usr/bin/env python3
"""UTide harmonic analysis for Nunavut CHS stations."""
...
q.QC_DIR = Path("/home/oliver/water_levels/Canada_IWLS_NU")
q.TEMPLATE_PATH = Path("/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt")
q.OUTPUT_PATH = Path("/home/oliver/harmonics/utide/harmonics_utide_canada_nu.txt")
q.CHECKPOINT_DIR = Path("/home/oliver/harmonics/utide/checkpoints_nu")

ORIG = q.format_station_block
def fmt(r):
    return ORIG(r).replace('# province: Québec','# province: Nunavut') \
                  .replace(', Québec, Canada',', Nunavut, Canada')
q.format_station_block = fmt

def main():
    q.CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)
    with open(q.QC_DIR/'_nu_stations.json') as f: cat=json.load(f)
    by_code={s['code']:s for s in cat}
    files=sorted(glob.glob(str(q.QC_DIR/'*_wlp.csv')))
    print(f"UTide — Nunavut ({len(files)} stations)\n")
    import utide
