#!/usr/bin/env python3
"""
UTide harmonic analysis for Iran stations from UHSLC hourly data.
Creates harmonics_utide_iran.txt (ISO-8859-1).

Stations: Chabahar (915), Jask (126)
Data: UHSLC Fast Delivery hourly sea level (mm, UTC)
Format: year,month,day,hour,level_mm (no header)
"""
import sys
...
OUTPUT_PATH = Path("/home/oliver/harmonics/utide/harmonics_utide_iran.txt")
DATA_DIR = Path("/home/oliver/water_levels/UHSLC_Iran")

# Direct CSV download (not ERDDAP)
...
def download_uhslc(station):
    """Download UHSLC hourly fast delivery CSV."""
    ...
    url = f"{UHSLC_CSV_BASE}/h{uhslc_id}.csv"
    print(f"  Download UHSLC {uhslc_id}...")
    try:
...
def load_uhslc_csv(csv_path):
    """Load UHSLC hourly CSV (year,month,day,hour,level_mm), return (datetimes_utc, levels_meters)."""
    times = []
    levels = []
    with open(csv_path) as f:
        ...
        for row in reader:
            ...
            try:
                ...
                continue
            if val < -9000 or val == -32767 or np.isnan(val):
                continue
            try:
...
def format_station_block(result):
    ...
    lines.append(f"{location}")
    lines.append(f"+03:30 :Asia/Tehran")
...
def main():
    print("=" * 70)
    print("UTide Harmonic Analysis -- Iran (UHSLC)")
    print("=" * 70)
    print()
