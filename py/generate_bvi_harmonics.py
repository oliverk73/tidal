#!/usr/bin/env python3
"""
Generate Harmonic Constants for British Virgin Islands using UTide.
Source: IOC SLSMF (Cane Garden/Tortola, code=tbvi2, rad sensor).
Only ~6 months of good data (Jun-Nov 2024). No UHSLC or PSMSL data available.
...
IOC_STATIONS = [
    {'ioc_code': 'tbvi2', 'name': 'Cane Garden Bay, Tortola', 'lat': 18.4283, 'lon': -64.6539,
     'water': 'Caribbean Sea', 'start': datetime(2024, 5, 15), 'end': datetime(2024, 8, 15),
     'sensor': 'rad'},
]
...
def load_ioc_csv(csv_path):
    ...
    # Remove values outside plausible range (sensor drifts above 4m after Jul 2024)
    v = df['waterlevel_m']
    n_before = len(df)
    df = df[(v > 2.5) & (v < 4.5)].copy()
    n_filtered = n_before - len(df)
    if n_filtered > 0:
        print(f"  Removed {n_filtered} implausible values ({100*n_filtered/n_before:.1f}%)")
    ...
    n_after = len(df)
    n_outliers = n_before - n_filtered - (len(df) - (n_before - n_filtered - n_after) if False else n_after)
    total_removed = n_before - n_after
    if total_removed > n_filtered:
        print(f"  Outliers removed: {total_removed - n_filtered}")
...
def format_station_block(station, results, data):
    name = station['name']
    full_name = f"{name}, British Virgin Islands"
    lines = []
    lines.append(f"# Harmonic constants derived from IOC SLSMF sea level data")
    ...
    lines.append(f"# country: British Virgin Islands")
    lines.append(f"# water body: {station['water']}")
    lines.append(f"# source: Derived from IOC SLSMF data (code={station['ioc_code']}, sensor={station['sensor']}) with UTide harmonic analysis")
...
def main():
    ioc_dir = Path('/home/oliver/water_levels/BVI_IOC')
    template_path = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt')
    output_path = Path('/home/oliver/harmonics/utide/harmonics_utide_bvi.txt')

    print("=" * 70)
    print("Generate British Virgin Islands Harmonics with UTide")
    print("=" * 70)
