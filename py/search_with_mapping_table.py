...
# Mapping table for Icelandic stations (CSV name → harmonics file name)
icelandic_mapping = {
    'Njarðvík': 'Njarvk',
    'Sandgerði': 'Sandgeri',
    'Grundarfjörður': 'Grundarfjrur',
    'Ísafjörður': 'safjrur',
    'Patreksfjörður': 'Patreksfjrur',
    'Siglufjörður': 'Siglufjrur',
    'Ólafsfjörður': 'lafsfjrur',
    'Seyðisfjörður': 'Seyisfjrur',
    'Hornafjörður': 'Hornafjrur'
}
...
for row in csv_data:
    if row.get('Status') == 'Nicht gefunden':
        ...
        # Check if this is an Icelandic station and map the name
        search_name = main_name
        if main_name in icelandic_mapping:
            search_name = icelandic_mapping[main_name]
            print(f"  Using mapped name: {search_name}")
                ...
                for i, line in enumerate(lines):
                    if not line.startswith('#') and not line.startswith('x '):
                        ...
                        if stripped and not stripped.startswith('+') and not stripped.startswith('-') and 'meters' not in stripped and 'feet' not in stripped and 'knots' not in stripped:
                            ...
                            # For mapped names, look for exact match of the mapped name
                            if search_name.lower() in stripped.lower():
                                is_match = True
