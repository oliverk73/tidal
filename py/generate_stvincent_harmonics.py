#!/usr/bin/env python3
"""
Generate Harmonic Constants for Saint Vincent & the Grenadines tide stations using UTide.
Sources: UHSLC (Kingstown, ID=795) + PSMSL CME (Calliaqua, hourly CSV).
...
# ── Station definitions ──────────────────────────────────────────────
UHSLC_STATIONS = [
    {'uhslc_id': 795, 'name': 'Kingstown', 'lat': 13.133, 'lon': -61.200, 'water': 'Caribbean Sea'},
]

PSMSL_STATIONS = [
    {'name': 'Calliaqua', 'lat': 13.1299, 'lon': -61.1955, 'water': 'Caribbean Sea',
     'csv_path': '/home/oliver/water_levels/StVincent_PSMSL/calq_rad.csv',
     'station_id': 'PSMSL-calq'},
]
...
# ── PSMSL CME data ──────────────────────────────────────────────────
def load_psmsl_csv(csv_path):
    df = pd.read_csv(csv_path, header=None, names=['time', 'waterlevel_m'])
    ...
    dt_list = [pd.Timestamp(t).to_pydatetime().replace(tzinfo=None) for t in df['time']]
...
def format_station_block(station, results, data, source_type='UHSLC'):
    ...
    if source_type == 'UHSLC':
        src_text = f"Derived from UHSLC hourly data (ID={station['uhslc_id']}) with UTide harmonic analysis"
        sid_text = f"UHSLC-{station['uhslc_id']}"
    else:
        src_text = f"Derived from PSMSL CME hourly data ({station['station_id']}) with UTide harmonic analysis"
        sid_text = station['station_id']
    lines.append(f"# Harmonic constants derived from {'UHSLC' if source_type == 'UHSLC' else 'PSMSL CME'} sea level data")
    ...
    lines.append(f"# station_id_context: {sid_text}")
...
def main():
    uhslc_dir = Path('/home/oliver/water_levels/StVincent_UHSLC')
    template_path = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt')
    output_path = Path('/home/oliver/harmonics/utide/harmonics_utide_stvincent.txt')

    print("=" * 70)
    print("Generate Saint Vincent & the Grenadines Harmonics with UTide")
    print("=" * 70)
    ...
    # ── PSMSL Stations ──
    print(f"\n{'='*40}")
    print("PSMSL CME Stations")
    print(f"{'='*40}")

    for station in PSMSL_STATIONS:
        name = station['name']
        ...
        block = format_station_block(station, results, data, source_type='PSMSL')
        station_blocks.append(block)
        processed += 1
