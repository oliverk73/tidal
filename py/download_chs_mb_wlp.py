#!/usr/bin/env python3
"""Download WLP from CHS IWLS API for Manitoba Hudson Bay stations."""
import os, json, time, datetime, sys, requests
import pandas as pd

API_BASE = 'https://api-sine.dfo-mpo.gc.ca/api/v1'
OUTPUT_DIR = '/home/oliver/water_levels/Canada_IWLS_MB'
CATALOG_PATH = os.path.join(OUTPUT_DIR, '_mb_stations.json')
RESOLUTION = 'FIFTEEN_MINUTES'
START_DATE = '2025-01-01'
END_DATE = '2025-12-31'
MB_CODES = ['04980', '05010']
...
def build_catalog():
    ...
    sel = [{'code':s['code'],'name':s['officialName'],'lat':s['latitude'],'lon':s['longitude'],'id':s['id']}
           for s in r.json() if s['code'] in MB_CODES]
...
def download_wlp(st):
    ...
    df.to_csv(p,index=False); print(f" {len(df)} Werte"); return True


def main():
    sts=build_catalog()
    print(f"{len(sts)} Manitoba-Stationen\n")
    for i,s in enumerate(sts):
        print(f"[{i+1}/{len(sts)}] {s['code']} {s['name']:<25s}",end=' ',flush=True)
        download_wlp(s)
