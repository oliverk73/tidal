#!/usr/bin/env python3
"""
Generate Harmonic Constants for Saint Kitts and Nevis using UTide.
Source: IOC SLSMF (Basseterre Coast Guard Base, code=bass, rad sensor).
Station was operational ~Dec 2013 to ~Mar 2018 (now down).
No UHSLC or PSMSL data available.
...
IOC_STATIONS = [
    {'ioc_code': 'bass', 'name': 'Basseterre', 'lat': 17.2900, 'lon': -62.7097, 'water': 'Caribbean Sea',
     'start': datetime(2013, 11, 1), 'end': datetime(2018, 3, 7), 'sensor': 'rad'},
]
...
def load_ioc_csv(csv_path):
    ...
    # Remove exact zeros (sensor dropouts)
    n_zeros = (df['waterlevel_m'] == 0).sum()
    if n_zeros > 0:
        df = df[df['waterlevel_m'] != 0].copy()
        print(f"  Removed {n_zeros} zero values (sensor dropouts)")
    v = df['waterlevel_m']
...
def format_station_block(station, results, data):
    name = station['name']
    full_name = f"{name}, Saint Kitts and Nevis"
    lines = []
    ...
    lines.append(f"# country: Saint Kitts and Nevis")
...
def main():
    ioc_dir = Path('/home/oliver/water_levels/StKitts_IOC')
    template_path = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt')
    output_path = Path('/home/oliver/harmonics/utide/harmonics_utide_stkitts.txt')

    print("=" * 70)
    print("Generate Saint Kitts and Nevis Harmonics with UTide")
    print("=" * 70)
