#!/usr/bin/env python3
"""
Generate Harmonic Constants for Aqaba, Jordan from IOC Sea Level Monitoring data.
Uses UTide for harmonic analysis. Outputs 175 XTide-compatible constituents.

Data source: https://www.ioc-sealevelmonitoring.org/station.php?code=aqab
Sensor: Radar, 5-minute sampling
Data available from ~2024-10-24 onwards.
"""
import numpy as np
...
def fetch_ioc_data(code, endtime, period=7):
    """Fetch data from IOC Sea Level Monitoring for a given period ending at endtime."""
...
def download_all_data(code, start_date, end_date, csv_path):
    """Download all data in 7-day chunks and save as CSV."""
    if csv_path.exists():
    ...
    print(f"  Downloading ~{total_chunks} chunks of {chunk_days} days each...")
    chunk_num = 0

    while current_end > start_date:
        chunk_num += 1
        endtime_str = current_end.strftime('%Y-%m-%d')
        print(f"  [{chunk_num}/{total_chunks}] {endtime_str}...", end='', flush=True)
    ...
    df = pd.DataFrame({
        'time': sorted_times,
        'waterlevel_m': [all_data[t] for t in sorted_times],
    })
...
def load_csv_water_levels(csv_path):
    df = pd.read_csv(csv_path)
    ...
    n_obs = len(dt_list)
    if n_obs < 2000:
        print(f"  Only {n_obs} observations - insufficient!")
        return None
    first = dt_list[0]
    last = dt_list[-1]
...
def format_station_block(station, results, data):
    ...
    lines = []
    lines.append(f"# Harmonic constants derived from IOC Sea Level Monitoring data")
...
def main():
    data_dir = Path('/home/oliver/water_levels/Aqaba_IOC')
    csv_path = data_dir / 'aqaba_ioc.csv'
    template_path = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt')
    output_path = Path('/home/oliver/harmonics/utide/harmonics_utide_jordan.txt')

    print("=" * 70)
    print("Generate Aqaba, Jordan Harmonics from IOC Sea Level Monitoring")
    print("=" * 70)

    # Step 1: Download data
    print(f"\nStep 1: Download sea level data from IOC...")
    result = download_all_data(STATION['ioc_code'], START_DATE, END_DATE, csv_path)
    if result is None:
        print("Failed to download data!")
        return

    # Step 2: Load data
    print(f"\nStep 2: Loading water level data...")
    data = load_csv_water_levels(csv_path)
    if data is None:
        print("Insufficient data for analysis!")
        return
    print(f"  {data['n_obs']} observations over {data['years']:.1f} years")
    print(f"  Range: {data['start_time']} to {data['end_time']}")

    # Step 3: UTide harmonic analysis
    print(f"\nStep 3: Running UTide harmonic analysis...")
    ...
    o1_amp = 0
    for c in results['constituents']:
        if c['name'] == 'M2': m2_amp = c['amplitude']
        if c['name'] == 'S2': s2_amp = c['amplitude']
        if c['name'] == 'K1': k1_amp = c['amplitude']
        if c['name'] == 'O1': o1_amp = c['amplitude']

    print(f"\n  Results:")
    print(f"  R² = {results['r_squared']:.4f}")
    print(f"  RMS error = {results['rms_error']:.4f} m")
    print(f"  Mean level = {results['mean']:.4f} m")
    print(f"  Constituents analyzed: {results['n_analyzed']}")
    print(f"  M2 = {m2_amp:.4f} m")
    print(f"  S2 = {s2_amp:.4f} m")
    print(f"  K1 = {k1_amp:.4f} m")
    print(f"  O1 = {o1_amp:.4f} m")

    # Step 4: Write output
    print(f"\nStep 4: Writing harmonics file...")
