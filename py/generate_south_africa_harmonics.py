#!/usr/bin/env python3
"""
Generate Harmonic Constants from UHSLC hourly water level data for South Africa.
...
STATIONS = [
    ...
    {'uhslc_id': 702, 'name': 'Luderitz',       'lat': -26.650, 'lon': 15.150, 'province': 'Namibia',        'water': 'Atlantic Ocean'},

    # --- West Coast (Atlantic) ---
    {'uhslc_id': 701, 'name': 'Port Nolloth',   'lat': -29.250, 'lon': 16.867, 'province': 'Northern Cape',  'water': 'Atlantic Ocean'},
    {'uhslc_id': 703, 'name': 'Saldanha Bay',   'lat': -33.017, 'lon': 17.950, 'province': 'Western Cape',   'water': 'Atlantic Ocean'},
    {'uhslc_id': 704, 'name': 'Cape Town',       'lat': -33.900, 'lon': 18.433, 'province': 'Western Cape',   'water': 'Atlantic Ocean'},
    {'uhslc_id': 221, 'name': "Simon's Town",    'lat': -34.183, 'lon': 18.433, 'province': 'Western Cape',   'water': 'Atlantic Ocean'},

    # --- South Coast (Indian Ocean) ---
    {'uhslc_id': 185, 'name': 'Mossel Bay',     'lat': -34.183, 'lon': 22.133, 'province': 'Western Cape',   'water': 'Indian Ocean'},
    {'uhslc_id': 186, 'name': 'Knysna',         'lat': -34.050, 'lon': 23.045, 'province': 'Western Cape',   'water': 'Indian Ocean'},
    {'uhslc_id': 184, 'name': 'Port Elizabeth',  'lat': -33.960, 'lon': 25.630, 'province': 'Eastern Cape',   'water': 'Indian Ocean'},
    {'uhslc_id': 187, 'name': 'East London',    'lat': -33.017, 'lon': 27.917, 'province': 'Eastern Cape',   'water': 'Indian Ocean'},

    # --- East Coast (Indian Ocean) ---
    {'uhslc_id': 181, 'name': 'Durban',         'lat': -29.867, 'lon': 31.050, 'province': 'KwaZulu-Natal',  'water': 'Indian Ocean'},
    {'uhslc_id': 188, 'name': "Richard's Bay",  'lat': -28.800, 'lon': 32.083, 'province': 'KwaZulu-Natal',  'water': 'Indian Ocean'},
]
...
def load_csv_water_levels(csv_path, max_years=19.0):
    ...
    # First: cut to the last max_years by timestamp (before NaN removal)
...
def format_station_block(station, results, data):
    ...
    country = 'Namibia' if province == 'Namibia' else 'South Africa'
...
def main():
    data_dir = Path('/home/oliver/water_levels/SouthAfrica_UHSLC')
    template_path = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt')
    output_path = Path('/home/oliver/harmonics/utide/harmonics_utide_south_africa.txt')

    print("=" * 70)
    print("Generate South Africa Harmonics with UTide")
    print("=" * 70)
