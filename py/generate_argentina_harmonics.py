#!/usr/bin/env python3
"""
Generate Harmonic Constants from UHSLC hourly water level data for Argentina.
...
STATIONS = [
    # --- Rio de la Plata ---
    {'uhslc_id': 285, 'name': 'Buenos Aires (Club de Pescadores)', 'lat': -34.561, 'lon': -58.399, 'province': 'Buenos Aires'},

    # --- Atlantic Coast ---
    {'uhslc_id': 729, 'name': 'Mar del Plata',   'lat': -38.035, 'lon': -57.538, 'province': 'Buenos Aires'},
    {'uhslc_id': 731, 'name': 'Puerto Madryn',   'lat': -42.763, 'lon': -65.030, 'province': 'Chubut'},
    {'uhslc_id': 286, 'name': 'Puerto Deseado',  'lat': -47.750, 'lon': -65.917, 'province': 'Santa Cruz'},

    # --- Tierra del Fuego ---
    {'uhslc_id': 600, 'name': 'Ushuaia',         'lat': -54.805, 'lon': -68.295, 'province': 'Tierra del Fuego'},

    # --- Antarctic ---
    {'uhslc_id': 601, 'name': 'Esperanza',       'lat': -63.395, 'lon': -56.995, 'province': 'Antartida Argentina'},
    {'uhslc_id': 682, 'name': 'Dallmann',        'lat': -62.233, 'lon': -58.683, 'province': 'Antartida Argentina'},
]
...
def format_station_block(station, results, data):
    ...
    lines.append(f"# country: Argentina")
...
def main():
    data_dir = Path('/home/oliver/water_levels/Argentina_UHSLC')
    ...
    print("=" * 70)
    print("Generate Argentina Harmonics with UTide")
    print("=" * 70)
