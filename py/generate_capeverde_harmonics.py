#!/usr/bin/env python3
"""
Generate Harmonic Constants for Cape Verde (Cabo Verde) using UTide.
Sources: UHSLC-235 (Palmeira, Sal).
Note: Mindelo (São Vicente) already exists in XTide. Praia has no available data.
...
UHSLC_STATIONS = [
    {'uhslc_id': 235, 'name': 'Palmeira, Sal', 'lat': 16.750, 'lon': -22.983, 'water': 'Atlantic Ocean'},
]
...
def format_station_block(station, results, data):
    ...
    lines.append(f"# country: Cape Verde")
    lines.append(f"# water body: {station['water']}")
    lines.append(f"# source: Derived from UHSLC hourly data (ID={station['uhslc_id']}) with UTide harmonic analysis")
...
def main():
    uhslc_dir = Path('/home/oliver/water_levels/CapeVerde_UHSLC')
    template_path = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt')
    output_path = Path('/home/oliver/harmonics/utide/harmonics_utide_capeverde.txt')

    print("=" * 70)
    print("Generate Cape Verde Harmonics with UTide")
    print("=" * 70)
