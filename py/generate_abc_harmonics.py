#!/usr/bin/env python3
"""
Generate Harmonic Constants for Aruba, Bonaire, Curaçao (ABC islands) using UTide.
Sources:
  - Curaçao: UHSLC-878 (Bullen Bay)
  - Bonaire: IOC bona (rad sensor, single column, data since ~2022)
  - Aruba: IOC oran (prs sensor, bat+prs columns, data ~Feb 2018 to Jun 2019)
Note: Bonaire is already in XTide (Kralendijk) but with many duplicates.
...
UHSLC_STATIONS = [
    {'uhslc_id': 878, 'name': 'Bullen Bay', 'lat': 12.187, 'lon': -69.020, 'water': 'Caribbean Sea',
     'country': 'Curaçao'},
]

IOC_STATIONS = [
    {'ioc_code': 'oran', 'name': 'Oranjestad', 'lat': 12.453, 'lon': -70.033, 'water': 'Caribbean Sea',
     'start': datetime(2017, 12, 1), 'end': datetime(2019, 7, 1), 'sensor': 'prs',
     'country': 'Aruba'},
    {'ioc_code': 'bona', 'name': 'Kralendijk', 'lat': 12.150, 'lon': -68.278, 'water': 'Caribbean Sea',
     'start': datetime(2022, 1, 1), 'end': datetime(2026, 4, 6), 'sensor': 'rad',
     'country': 'Bonaire'},
]
...
def format_station_block(station, results, data, source_type='UHSLC'):
    ...
    if source_type == 'UHSLC':
        ...
        src_text = f"Derived from IOC SLSMF data (code={station['ioc_code']}, sensor={station.get('sensor','rad')}) with UTide harmonic analysis"
        sid_text = f"IOC-{station['ioc_code']}"
...
def main():
    uhslc_dir = Path('/home/oliver/water_levels/ABC_UHSLC')
    ioc_dir = Path('/home/oliver/water_levels/ABC_IOC')
    template_path = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt')
    output_path = Path('/home/oliver/harmonics/utide/harmonics_utide_abc.txt')

    print("=" * 70)
    print("Generate Aruba/Bonaire/Curaçao Harmonics with UTide")
    print("=" * 70)
    ...
    for station in UHSLC_STATIONS:
        ...
        name = station['name']
        print(f"\n[UHSLC-{uid}] {name} ({station['country']})...")
    ...
    for station in IOC_STATIONS:
        ...
        print(f"\n[IOC-{code}] {name} ({station['country']}, sensor={sensor})...")
