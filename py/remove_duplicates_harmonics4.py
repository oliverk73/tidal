...
def find_station_blocks(lines):
    ...
    for idx, station_line in enumerate(station_lines):
        # Find start of comment block by going backwards until we hit non-comment content
        start = station_line
        for j in range(station_line - 1, max(0, station_line - 100), -1):
            line = lines[j].strip()
            if line.startswith('#'):
                # This is a comment line - include it in the block
                start = j
            elif not line:
                # Empty line - continue looking back
                start = j
            else:
                # Non-comment, non-empty line (e.g., "x 0 0" or constituent data)
                # This is the end of the previous station's data
                break
...
def main():
    ...
    new_file = os.path.join(base_dir, 'harmonics-2004-06-14.txt')
    old_file = os.path.join(base_dir, 'HARMONICS_NO_US_no_us_no_dupes2.txt')
    output_file = os.path.join(base_dir, 'HARMONICS_NO_US_no_us_no_dupes3.txt')
    csv_file = os.path.join(base_dir, 'deleted_stations_2004.csv')
