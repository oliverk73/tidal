#!/usr/bin/env python3
"""
Generate Harmonic Constants for Costa Rican tide stations using UTide.
Sources: UHSLC (Cocos Island, Quepos, Limon)
       + IOC SLSMF (Golfito, Flamingo Marina, Cuajiniquil).
...
# ── Station definitions ──────────────────────────────────────────────
UHSLC_STATIONS = [
    {'uhslc_id': 392, 'name': 'Cocos Island',  'lat': 5.557,  'lon': -87.048, 'water': 'Pacific Ocean'},
    {'uhslc_id': 87,  'name': 'Quepos',         'lat': 9.400,  'lon': -84.167, 'water': 'Pacific Ocean'},
...
def format_station_block(station, results, data, source_label):
    ...
    lines.append(f"# country: Costa Rica")
...
def main():
    uhslc_dir = Path('/home/oliver/water_levels/CostaRica_UHSLC')
    ioc_dir = Path('/home/oliver/water_levels/CostaRica_IOC')
    template_path = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt')
    output_path = Path('/home/oliver/harmonics/utide/harmonics_utide_costarica.txt')

    print("=" * 70)
    print("Generate Costa Rica Harmonics with UTide")
    print("=" * 70)
    ...
    for station in UHSLC_STATIONS:
        ...
        csv_path = uhslc_dir / f'{sid}_cr.csv'
