#!/usr/bin/env python3
"""
Generate Harmonic Constants from UHSLC hourly water level data for New Zealand.
...
STATIONS = [
    {'uhslc_id': 398, 'name': 'Marsden Point',  'lat': -35.833, 'lon': 174.500, 'region': 'Northland',       'water': 'Pacific Ocean'},
    {'uhslc_id':  70, 'name': 'Auckland',        'lat': -36.850, 'lon': 174.767, 'region': 'Auckland',        'water': 'Hauraki Gulf'},
    {'uhslc_id':  73, 'name': 'Tauranga',        'lat': -37.650, 'lon': 176.183, 'region': 'Bay of Plenty',   'water': 'Pacific Ocean'},
    {'uhslc_id':  78, 'name': 'Gisborne',        'lat': -38.683, 'lon': 178.033, 'region': 'Gisborne',        'water': 'Pacific Ocean'},
    {'uhslc_id': 668, 'name': 'Napier',          'lat': -39.483, 'lon': 176.917, 'region': "Hawke's Bay",     'water': 'Pacific Ocean'},
    {'uhslc_id':  76, 'name': 'Taranaki',        'lat': -39.050, 'lon': 174.033, 'region': 'Taranaki',        'water': 'Tasman Sea'},
    {'uhslc_id':  75, 'name': 'Wanganui',        'lat': -39.947, 'lon': 174.990, 'region': 'Manawatu-Whanganui', 'water': 'Tasman Sea'},
    {'uhslc_id':  71, 'name': 'Wellington',       'lat': -41.283, 'lon': 174.783, 'region': 'Wellington',      'water': 'Cook Strait'},
    {'uhslc_id':  77, 'name': 'Nelson',           'lat': -41.267, 'lon': 173.267, 'region': 'Nelson',          'water': 'Tasman Bay'},
    {'uhslc_id':  74, 'name': 'Westport',         'lat': -41.733, 'lon': 171.600, 'region': 'West Coast',      'water': 'Tasman Sea'},
    {'uhslc_id': 403, 'name': 'Jackson Bay',      'lat': -43.983, 'lon': 168.617, 'region': 'West Coast',      'water': 'Tasman Sea'},
    {'uhslc_id': 667, 'name': 'Lyttelton',        'lat': -43.600, 'lon': 172.717, 'region': 'Canterbury',      'water': 'Pacific Ocean'},
    {'uhslc_id': 665, 'name': 'Timaru',           'lat': -44.383, 'lon': 171.250, 'region': 'Canterbury',      'water': 'Pacific Ocean'},
    {'uhslc_id': 669, 'name': 'Port Chalmers',    'lat': -45.817, 'lon': 170.650, 'region': 'Otago',           'water': 'Pacific Ocean'},
    {'uhslc_id':  72, 'name': 'Bluff',            'lat': -46.600, 'lon': 168.350, 'region': 'Southland',       'water': 'Foveaux Strait'},
    {'uhslc_id':  79, 'name': 'Chatham Island',   'lat': -43.947, 'lon': -176.562, 'region': 'Chatham Islands', 'water': 'Pacific Ocean'},
    {'uhslc_id': 663, 'name': 'Scott Base',       'lat': -77.850, 'lon': 166.767, 'region': 'Ross Island',     'water': 'Ross Sea'},
]
...
def load_csv_water_levels(csv_path, max_years=10.0):
    ...
    Limits to max_years using the most recent data."""
...
def format_station_block(station, results, data):
    ...
    lon = station['lon']
    water = station['water']
    full_name = f"{name}, {region}, New Zealand"
    ...
    lines.append(f"# country: New Zealand")
...
def read_header_from_template(template_path):
    """Read header from template file up to first station block."""
...
def main():
    data_dir = Path('/home/oliver/water_levels/New_Zealand_UHSLC')
    template_path = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt')
    output_path = Path('/home/oliver/harmonics/utide/harmonics_utide_new_zealand.txt')

    print("=" * 70)
    print("Generate New Zealand Harmonics with UTide")
    print("=" * 70)
