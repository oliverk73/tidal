...
SOURCE_GROUPS = OrderedDict([
    ('Harmonics DWF 2025', {'color': '#2196F3', 'files': ['harmonics-dwf-20251228-free.tcd']}),
    ('Harmonics DWF 2010', {'color': '#607D8B', 'files': ['harmonics-dwf-20100529-nonfree_mod.tcd']}),
    ('Harmonics DWF 2007', {'color': '#795548', 'files': ['harmonics-dwf-20070318_mod.tcd']}),
...
GROUP_NAMES = list(SOURCE_GROUPS.keys()) + ['Other']
GROUP_COLOR = {g: SOURCE_GROUPS[g]['color'] for g in SOURCE_GROUPS}
GROUP_COLOR['Other'] = '#999999'
...
(function () {
  ...
  function buildAll(data) {
    ...
    // Fuer die Suche (static/js/suche.js): alle Saetze, auch gleichnamige.
    window.stationRows = stations;
    window.stationGroups = groups;
  ...
  function addStationControl(tideGroups, currentGroups, tideTotal, currentTotal) {
    ...
    function setupMaster(root, idPrefix, groups) {
      var master = root.querySelector('[data-master="' + idPrefix + '"]');
      if (!master) return;
      var subs = [];
      for (var i = 0; i < groups.length; i++) {
        subs.push(root.querySelector('[data-group="' + idPrefix + '-' + i + '"]'));
      }
    ...
    var tideHtml = buildSection('All (' + tideTotal + ')', tideHeaderSvg, tideGroups, 'tide');
    var currentHtml = buildSection('All (' + currentTotal + ')', currentHeaderSvg, currentGroups, 'current');

    // Primaer: zwei getrennte Panels "Tide Stations" / "Current Stations" (templates/index.html).
    var tideBody = document.getElementById('tide-stations-body');
    var currentBody = document.getElementById('current-stations-body');
    if (tideBody && currentBody) {
      tideBody.innerHTML = tideHtml;
      currentBody.innerHTML = currentHtml;
      setupMaster(tideBody, 'tide', tideGroups);
      setupMaster(currentBody, 'current', currentGroups);
      var tc = document.getElementById('tide-stations-count');
      if (tc) tc.textContent = '(' + tideTotal + ')';
      var cc = document.getElementById('current-stations-count');
      if (cc) cc.textContent = '(' + currentTotal + ')';
      return;
    }

    // Fallback: eine Box unten rechts, falls die Panels fehlen.
    var html = tideHtml + '<div style="border-top:1px solid rgba(0,0,0,0.15);margin:5px 0;"></div>' + currentHtml;
    var StationControl = L.Control.extend({
      ...
      onAdd: function () {
        ...
        div.innerHTML = html;
        setupMaster(div, 'tide', tideGroups);
        setupMaster(div, 'current', currentGroups);
        return div;
      }
    });
