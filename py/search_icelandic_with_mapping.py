...
# Function to normalize Icelandic names for comparison
def normalize_icelandic_name(name):
    """Remove or replace Icelandic special characters for matching"""
    # Remove accents and normalize
    normalized = unicodedata.normalize('NFD', name)
    # Remove combining characters
    normalized = ''.join(char for char in normalized if unicodedata.category(char) != 'Mn')
    # Replace specific Icelandic characters
    replacements = {
        'ð': 'd',
        'Ð': 'D',
        'þ': 'th',
...
for row in csv_data:
    if row.get('Status') == 'Nicht gefunden':
        ...
        normalized_search = normalize_icelandic_name(station_name)
        search_parts = station_name.split(',')
        main_name = search_parts[0].strip()
        normalized_main = normalize_icelandic_name(main_name)

        print(f"  Normalized: {normalized_main}")
                ...
                for i, line in enumerate(lines):
                    if not line.startswith('#') and not line.startswith('x '):
                        ...
                        if stripped and not stripped.startswith('+') and not stripped.startswith('-') and 'meters' not in stripped and 'feet' not in stripped and 'knots' not in stripped:
                            ...
                            normalized_line = normalize_icelandic_name(stripped)

                            # Check for match
                            is_match = False
                            if normalized_main in normalized_line and len(normalized_main) > 3:
                                is_match = True
