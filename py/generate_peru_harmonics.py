#!/usr/bin/env python3
"""
Generate Harmonic Constants for Peruvian tide stations using UTide.
Sources: UHSLC (Callao, Lobos de Afuera, Matarani, Talara)
       + IOC SLSMF (Chala, Chancay, Chimbote, Ilo, Lobos de Chiclayo).
...
# ── Station definitions ──────────────────────────────────────────────
UHSLC_STATIONS = [
    {'uhslc_id': 92,  'name': 'Talara',           'lat': -4.582,  'lon': -81.282, 'water': 'Pacific Ocean'},
    {'uhslc_id': 84,  'name': 'Lobos de Afuera',  'lat': -6.933,  'lon': -80.717, 'water': 'Pacific Ocean'},
    {'uhslc_id': 93,  'name': 'Callao',            'lat': -12.050, 'lon': -77.150, 'water': 'Pacific Ocean'},
    {'uhslc_id': 94,  'name': 'Matarani',          'lat': -17.002, 'lon': -72.108, 'water': 'Pacific Ocean'},
]

IOC_STATIONS = [
    {'ioc_code': 'lobo2', 'name': 'Isla Lobos de Tierra', 'lat': -6.9513, 'lon': -80.7194, 'water': 'Pacific Ocean',
     'start': datetime(2025, 6, 1), 'end': datetime(2026, 4, 6)},
    {'ioc_code': 'chim2', 'name': 'Chimbote',             'lat': -9.0762, 'lon': -78.6127, 'water': 'Pacific Ocean',
     'start': datetime(2025, 9, 1), 'end': datetime(2026, 4, 6)},
    {'ioc_code': 'chan',   'name': 'Chancay',              'lat': -11.5849, 'lon': -77.2732, 'water': 'Pacific Ocean',
     'start': datetime(2025, 8, 1), 'end': datetime(2026, 4, 6)},
    {'ioc_code': 'chala', 'name': 'Chala',                 'lat': -15.8661, 'lon': -74.2481, 'water': 'Pacific Ocean',
     'start': datetime(2023, 1, 1), 'end': datetime(2026, 4, 6)},
    {'ioc_code': 'ilom2', 'name': 'Ilo',                   'lat': -17.6447, 'lon': -71.3496, 'water': 'Pacific Ocean',
     'start': datetime(2025, 6, 1), 'end': datetime(2026, 4, 6)},
]
...
def main():
    uhslc_dir = Path('/home/oliver/water_levels/Peru_UHSLC')
    ioc_dir = Path('/home/oliver/water_levels/Peru_IOC')
    template_path = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt')
    output_path = Path('/home/oliver/harmonics/utide/harmonics_utide_peru.txt')

    print("=" * 70)
    print("Generate Peru Harmonics with UTide")
    print("=" * 70)
    ...
    for station in UHSLC_STATIONS:
        ...
        csv_path = uhslc_dir / f'{sid}_peru.csv'
