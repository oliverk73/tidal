...
REFMAP = {
 ...
 # --- Americas volumes (ectt/wctt 2020) ---
 'Valparaiso': 'Valparaíso',                 # Chile (NICHT Valparaiso, Florida!)
 'Quebec': 'Lauzon',                          # Lauzon = Quebec City tide reference
 'Puerto Ingeniero White': 'Ingeniero White', # Bahía Blanca, Argentina
 'Cabo de Hornos': 'Cabo de Hornos',          # aus Table-1 gefittet (harmonics_noaa_refs)
 'San Cristobal': 'San Crist',                # Galápagos (Table-1)
 'St. Georges Island': 'St. Georges Island',  # Bermuda (Table-1)
 'Settlement Point': 'Settlement Point',       # Bahamas
 'Suriname Rivier': 'Suriname Rivier',
 'Harrington Harbour': 'Harrington Harbour',
...
def tz_lookup(lat, lon):
    ...
    if z and z.startswith('America/Argentina/'): z = 'America/Buenos_Aires'  # UTC-3, libtcd 29-char limit
    if z and len(z) <= 29: return z
    if z: return z[:29]
...
def block(s, tr, cty, OV=None, EXISTING=None):
    ...
    _vl = {'ectt': 'East Coast of N & S America', 'wctt': 'West Coast of N & S America'}.get(s.get('_vol'), 'Americas')
    note = (f"NOAA Tide Tables (C&GS/NOS) {_vl} 2020 Table 2 transfer from "
...
def main():
    ...
    print(f"skipped {len(skipped)}:", dict(Counter(r.split(':')[0] for _, r in skipped)))
    noref = [r.split(':',1)[1] for _, r in skipped if r.startswith('noref')]
    if noref: print('  NOREF refs:', dict(Counter(noref)))
