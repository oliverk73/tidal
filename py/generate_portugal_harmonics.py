#!/usr/bin/env python3
"""
Generate Harmonic Constants for Portuguese tide stations using UTide.
Sources: UHSLC (Lagos, Horta) + IOC (Setúbal, Albufeira, Arrifana, Sagres, Porto Santo, Lagos)
...
IOC_STATIONS = [
    {'ioc_code': 'albu', 'name': 'Albufeira', 'country': 'Portugal',
     ...
     'csv': '/home/oliver/water_levels/Portugal_IOC/albu_Albufeira.csv'},
    {'ioc_code': 'arri', 'name': 'Arrifana', 'country': 'Portugal',
     'lat': 37.2956, 'lon': -8.8694, 'water': 'Atlantic Ocean',
     'csv': '/home/oliver/water_levels/Portugal_IOC/arri_Arrifana.csv'},
    {'ioc_code': 'sagr', 'name': 'Sagres', 'country': 'Portugal',
     'lat': 37.0104, 'lon': -8.9282, 'water': 'Atlantic Ocean',
     'csv': '/home/oliver/water_levels/Portugal_IOC/sagr_Sagres.csv'},
    {'ioc_code': 'posa', 'name': 'Porto Santo', 'country': 'Madeira, Portugal',
     'lat': 33.0597, 'lon': -16.3149, 'water': 'Atlantic Ocean',
     'csv': '/home/oliver/water_levels/Portugal_IOC/posa_Porto_Santo.csv'},
    {'ioc_code': 'lgos', 'name': 'Lagos (IOC)', 'country': 'Portugal',
     'lat': 37.0988, 'lon': -8.6668, 'water': 'Atlantic Ocean',
     'csv': '/home/oliver/water_levels/Portugal_IOC/lgos_Lagos.csv',
     'skip_if_uhslc': True},  # Only use if UHSLC Lagos fails
]
...
def load_ioc_csv(csv_path):
    ...
    v = df['level_m']
    df = df[(v > -5) & (v < 5)].copy()
    # Remove outliers (3 passes, 3.5-sigma)
    for _ in range(3):
        v = df['level_m']
...
def format_station_block(station, results, data, source_label):
    name = station['name']
    # Remove "(IOC)" suffix for display
    display_name = name.replace(' (IOC)', '')
    full_name = f"{display_name}, {station['country']}"
    lines = []
    ...
    lines.append(f"# country: Portugal")
    ...
    lines.append(f"# !longitude: {station['lon']:.4f}")
    lines.append(f"# !latitude: {station['lat']:.4f}")
...
def main():
    ...
    output_path = Path('/home/oliver/harmonics/utide/harmonics_utide_portugal.txt')

    print("=" * 70)
    print("Generate Portuguese Harmonics with UTide")
    print("=" * 70)
    ...
    for station in IOC_STATIONS:
        ...
        # Skip IOC Lagos if UHSLC Lagos succeeded
        if station.get('skip_if_uhslc') and lagos_done:
            print(f"\n[IOC-{code}] {name} — SKIPPED (UHSLC version used)")
            continue
