#!/usr/bin/env python3
"""
Generate Harmonic Constants for Montserrat tide station using UTide.
Source: IOC SLSMF (Little Bay, code=mnts, prs sensor).
Station operational since 2024-03-22, operated by UKHO/OceanWise.
No UHSLC or PSMSL data available for Montserrat.
...
IOC_STATIONS = [
    {'ioc_code': 'mnts', 'name': 'Little Bay', 'lat': 16.8035, 'lon': -62.2059, 'water': 'Caribbean Sea',
     'start': datetime(2024, 3, 20), 'end': datetime(2026, 4, 6), 'sensor': 'prs'},
]
...
def fetch_ioc_data(code, endtime, period=7, sensor='prs'):
    ...
    # Find header row and target sensor column
    sensor_col = None
...
def download_ioc_data(code, start_date, end_date, csv_path, sensor='prs'):
    if csv_path.exists():
...
def format_station_block(station, results, data):
    ...
    lines.append(f"# country: Montserrat")
...
def main():
    ioc_dir = Path('/home/oliver/water_levels/Montserrat_IOC')
    template_path = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt')
    output_path = Path('/home/oliver/harmonics/utide/harmonics_utide_montserrat.txt')

    print("=" * 70)
    print("Generate Montserrat Harmonics with UTide")
    print("=" * 70)
