#!/usr/bin/env python3
"""
Generate Harmonic Constants for Nicaraguan tide stations using UTide.
Source: IOC SLSMF (Puerto Corinto).
Nicaragua has no UHSLC stations. Only Puerto Corinto (pcrt) has sufficient IOC data.
...
IOC_STATIONS = [
    {'ioc_code': 'pcrt', 'name': 'Puerto Corinto', 'lat': 12.483, 'lon': -87.167, 'water': 'Pacific Ocean',
     'start': datetime(2019, 9, 1), 'end': datetime(2026, 4, 6)},
]
...
def format_station_block(station, results, data):
    ...
    lines.append(f"# country: Nicaragua")
...
def main():
    ioc_dir = Path('/home/oliver/water_levels/Nicaragua_IOC')
    template_path = Path('/home/oliver/harmonics/utide/harmonics_utide_canada_all.txt')
    output_path = Path('/home/oliver/harmonics/utide/harmonics_utide_nicaragua.txt')

    print("=" * 70)
    print("Generate Nicaragua Harmonics with UTide")
    print("=" * 70)
    ...
    for station in IOC_STATIONS:
        ...
        print(f" OK (R²={results['r_squared']:.3f}, M2={m2:.3f}m, K1={k1:.3f}m, S2={s2:.3f}m, constit={results['n_analyzed']})")
