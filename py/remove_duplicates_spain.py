#!/usr/bin/env python3
"""
Script to remove duplicate stations from harmonics_old_no_us_no_dupes3.txt that already exist
in harmonics_puertos_spain.txt.

Duplicate criteria:
1. Exact match of location name line
2. Same location name + country (España/Spain treated as equivalent)
...
6. Coordinates within 2km (regardless of name)
"""
...
def parse_harmonics_file(filepath):
    ...
    station_pattern = re.compile(r'^[A-Z][^#\n]*[a-zA-Z\)ñü]$')
...
def is_duplicate(station1, station2):
    ...
    # Criterion 6: Coordinates within 2km (regardless of name)
    if (station1['latitude'] is not None and station1['longitude'] is not None and
        station2['latitude'] is not None and station2['longitude'] is not None):
        distance = haversine_distance(
            station1['latitude'], station1['longitude'],
        ...
        if distance < 2.0:
            return True, f"Coords within {distance:.2f}km: {name1} vs {name2}"
