#!/usr/bin/env python3
"""
UTide harmonic analysis for Danish tide stations using DMI Open Data.
10-min observations, DVR90 datum. Checkpoint/resume per station.
"""
import sys
...
NPZ_DIR = Path("/home/oliver/water_levels/Denmark_DMI/npz")
TEMPLATE_PATH = Path("/home/oliver/harmonics_working/harmonics-dwf-20070318_no_us_no_dupes.txt")
OUTPUT_PATH = Path("/home/oliver/harmonics_working/harmonics_utide_denmark.txt")
CHECKPOINT_DIR = Path("/home/oliver/harmonics_working/checkpoints_dk")
...
def format_station_block(result):
    ...
    lines = []
    lines.append(f"# Harmonic constants derived from DMI 10-min observation data")
    ...
    lines.append(f"# country: Denmark")
    lines.append(f"# source: Derived from DMI observed data with UTide harmonic analysis")
    lines.append(f"# station_id_context: DMI-{station['station_id']}")
    lines.append(f"# date_imported: {datetime.now().strftime('%Y%m%d')}")
    lines.append(f"# datum: DVR90")
    ...
    lines.append(f"+01:00 :Europe/Copenhagen")
...
def main():
    ...
    print("=" * 70)
    print("UTide Harmonic Analysis — Denmark (DMI)")
    print(f"10-min Beobachtungen, DVR90-Datum, letzte {USE_LAST_N_YEARS} Jahre")
    print("=" * 70)
